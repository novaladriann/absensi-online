...
<div id="guruModal" class="warn-modal-overlay" style="display:none;">
    <div class="warn-modal-box koreksi-modal-box guru-modal-box">
        ...
        <div id="guruModalAlert" class="alert-theme" style="display:none; margin-bottom:14px;"></div>
...
<div id="guruKelasModal" class="warn-modal-overlay" style="display:none;">
    <div class="warn-modal-box koreksi-modal-box guru-modal-box">
        ...
        <div id="guruKelasModalAlert" class="alert-theme" style="display:none; margin-bottom:14px;"></div>
...
<script>
    (function() {
        ...
        const guruModalAlert = document.getElementById('guruModalAlert');
        const guruKelasModalAlert = document.getElementById('guruKelasModalAlert');
        ...
        function showAlert(message, type = 'success', target = 'page') {
            ...
            if (target === 'modal') {
                el = guruModalAlert;
            } else if (target === 'kelas-modal') {
                el = guruKelasModalAlert;
            }
        ...
        function hideGuruModalAlert() {
            if (guruModalAlert) {
                guruModalAlert.style.display = 'none';
                guruModalAlert.innerHTML = '';
                guruModalAlert.className = 'alert-theme';
            }
        }

        function hideGuruKelasModalAlert() {
            if (guruKelasModalAlert) {
                guruKelasModalAlert.style.display = 'none';
                guruKelasModalAlert.innerHTML = '';
                guruKelasModalAlert.className = 'alert-theme';
            }
        }
        ...
        guruForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (!data.success) {
                    showAlert(data.message || 'Gagal menyimpan data guru.', 'error', 'modal');
                    return;
                }

                closeGuruModal();
                showAlert(data.message || 'Data guru berhasil disimpan.', 'success', 'page');
                loadGuruTable();
            } catch (err) {
                showAlert(err.message || 'Terjadi kesalahan saat menyimpan data guru.', 'error', 'modal');
            } finally {
        ...
        guruKelasForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (!data.success) {
                    showAlert(data.message || 'Gagal menyimpan relasi kelas guru.', 'error', 'kelas-modal');
                    return;
                }

                closeGuruKelasModal();
                showAlert(data.message || 'Relasi kelas guru berhasil diperbarui.', 'success', 'page');
                loadGuruTable();
            } catch (err) {
                showAlert(err.message || 'Terjadi kesalahan saat menyimpan relasi kelas.', 'error', 'kelas-modal');
            } finally {
