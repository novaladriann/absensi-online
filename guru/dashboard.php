                    ...
                    <div class="guru-chart-header">
                        <div>
                            ...
                            <div class="guru-chart-sub">
                                Tanggal: <strong id="labelTanggalDashboard">-</strong><br>
                                Kelas: <strong id="labelKelasDashboard">-</strong>
                            </div>
                        </div>
...
const labelTanggalDashboard = document.getElementById('labelTanggalDashboard');
const labelKelasDashboard = document.getElementById('labelKelasDashboard');
const guruDashboardDesc = document.getElementById('guruDashboardDesc');

let currentTanggal = new URLSearchParams(window.location.search).get('tanggal') || new Date().toISOString().split('T')[0];
...
function updateUrl(tanggal) {
    const url = new URL(window.location);
    url.searchParams.set('tanggal', tanggal);
    window.history.replaceState({}, '', url);
}

async function loadGuruDashboard(tanggal) {
    try {
        refreshBtn.disabled = true;
        refreshBtn.classList.add('is-loading');

        const response = await fetch(`<?= BASE_URL; ?>/guru/dashboard_data.php?tanggal=${encodeURIComponent(tanggal)}`);
        ...
        labelTanggalDashboard.textContent = data.labelTanggal;
        labelKelasDashboard.textContent = data.daftarKelas;
        guruDashboardDesc.textContent = `Ringkasan aktivitas siswa pada tanggal ${data.labelTanggal}.`;
...
dateInput.addEventListener('change', function () {
    if (this.value) {
        loadGuruDashboard(this.value);
    }
});

refreshBtn.addEventListener('click', function () {
    loadGuruDashboard(currentTanggal);
});
